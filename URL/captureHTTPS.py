...
class wireshark_analysis_script():
    ...
    def follow_tcp_stream(self, packets_file_obj, ip, port):
        for tmp_packet in packets_file_obj:
            ...
            print(highest_layer_name)
            if(highest_layer_name == "TLS"):
                for key in tmp_packet.tls._all_fields.keys():
                    if key == "tls.handshake.extensions_server_name":
                        print(tmp_packet.tls._all_fields[key])
            if (highest_layer_name == "SSL"):
                for key in tmp_packet.ssl._all_fields.keys():
                    if key == "ssl.handshake.extensions_server_name":
                        print(tmp_packet.tls._all_fields[key])
            # if ((tmp_packet.ip.dst == ip) and (tmp_packet.tcp.dstport == port)):
            #     print("server(%s:%s)->client(%s:%s): %s" % (
            #     tmp_packet.ip.src, tmp_packet.tcp.srcport, tmp_packet.ip.dst, tmp_packet.tcp.dstport,
            #     tmp_packet[highest_layer_name].get_field('data')))
            # elif ((tmp_packet.ip.src == ip) and (tmp_packet.tcp.srcport == port)):
            #     print("client(%s:%s)->server(%s:%s): %s" % (
            #     tmp_packet.ip.src, tmp_packet.tcp.srcport, tmp_packet.ip.dst, tmp_packet.tcp.dstport,
            #     tmp_packet[highest_layer_name].get_field('data')))
...
if __name__ == '__main__':
    ...
    packets_file_path = './caps/test.pcapng'
    # tshark程序所在的路径，tshark随wireshark安装
    tshark_path = 'C:\\Program Files\\Wireshark\\tshark.exe'
    # 过滤器表达式，与在wireshark中使用时的写法完全相同
    first_step_filter = 'ssl contains "baidu"'
    # 用于存放要追踪流的ip和端口
    ...
    for target_client_ip_port_temp in target_client_ip_port:
        ...
        second_step_filter = 'ssl and ip.addr == %s and ip.addr == %s  and tcp.port == %s and tcp.port == %s' % (
        ip_server, ip_client, port_server, port_client)
        second_step_obj = wireshark_analysis_script_instance.read_packets_from_file(packets_file_path, tshark_path,
                                                                                    second_step_filter)
